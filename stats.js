// stats.js - EXACTE EXCEL VBA MACRO KLASSEMENT MET 4/3/2/1 PUNTENSYSTEEM
// ==================== INITIALISATIE ====================
...
// ==================== EXACTE MACRO LOGICA MET 4/3/2/1 PUNTEN ====================
...
// Bereken matchpunten per speler per speeldag MET 4/3/2/1 SYSTEEM
...
// 🎯 BEREKEN MATCHPUNTEN VOLGENS DE JUISTE REGELS (4/3/2/1)
let matchpunten;
if (isWinnaar) {
...
// ==================== SPELER DETAIL FUNCTIES MET 4/3/2/1 PUNTEN ====================
...
// 🎯 BEREKEN MATCHPUNTEN MET 4/3/2/1 SYSTEEM
const matchpunten = gewonnen ? 
  (gemiddeldePerPartij >= tsg ? 4 : 2) : 
  (gemiddeldePerPartij >= tsg ? 3 : 1);
resultRows.push({
...
const hoogsteReeks = isPlayer1 ? (match.p1Highest || 0) : (match.p2Highest || 0);
// 🎯 BEREKEN MATCHPUNTEN MET 4/3/2/1 SYSTEEM
const turnsCount = isPlayer1 ? (match.p1Turns?.length || 0) : (match.p2Turns?.length || 0);
const gespeeldGem = turnsCount > 0 ? (eigenScore / turnsCount) : 0;
const player = getPlayers().find(p => p.name === spelerNaam);
const tsg = player?.tsg ? parseFloat(player.tsg.replace(',', '.')) : 0;
const matchpunten = gewonnen ? 
  (gespeeldGem >= tsg ? 4 : 2) : 
  (gespeeldGem >= tsg ? 3 : 1);
...
console.log('🎯 Excel VBA Macro stats.js functies geregistreerd - MET 4/3/2/1 PUNTENSYSTEEM');
